...
-- שדות נוספים לזיהוי מוצרים פנימיים ונורמליזציה
ALTER TABLE public.items ADD COLUMN IF NOT EXISTS item_type SMALLINT DEFAULT 1;           -- 0=ברקוד פנימי, 1=ברקוד תקני (EAN)
ALTER TABLE public.items ADD COLUMN IF NOT EXISTS is_weighted BOOLEAN DEFAULT false;       -- מוצר שקיל?
ALTER TABLE public.items ADD COLUMN IF NOT EXISTS quantity REAL;                            -- כמות ביחידה
ALTER TABLE public.items ADD COLUMN IF NOT EXISTS unit_qty VARCHAR(20) DEFAULT '';          -- UnitQty מה-XML
ALTER TABLE public.items ADD COLUMN IF NOT EXISTS qty_in_package SMALLINT DEFAULT 1;       -- כמות יחידות באריזה
ALTER TABLE public.items ADD COLUMN IF NOT EXISTS unit_of_measure_price REAL;               -- מחיר ליחידת מידה
ALTER TABLE public.items ADD COLUMN IF NOT EXISTS normalized_name TEXT DEFAULT '';           -- שם מנורמל להתאמת ברקודים פנימיים

-- אינדקסים לחיפוש מוצרים פנימיים לפי שם מנורמל
CREATE INDEX IF NOT EXISTS idx_items_normalized_name ON items(normalized_name) WHERE item_type = 0;
CREATE INDEX IF NOT EXISTS idx_items_item_type ON items(item_type);


-- =============================================
-- 3b. מיגרציה: נורמליזציית יחידות מידה
-- =============================================
...
-- View: השוואת מחירים (כולל שם הרשת מטבלת chains + שדות נורמליזציה)
DROP VIEW IF EXISTS price_comparison;
